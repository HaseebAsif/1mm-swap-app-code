...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    ...
    // trigger withdrawAll function if the withdrawal will leave 0.000001 1MM or less
    ...
    if (isWithdrawingAll) {
      try {
        const tx = await dexTokenVaultContract.withdrawAll(callOptions)
      ...
      try {
        const tx = await dexTokenVaultContract.withdraw(shareStakeToWithdraw.sharesAsBigNumber.toString(), callOptions)
        const receipt = await tx.wait()
        if (receipt.status) {
          toastSuccess(t('Unstaked!'), t('Your earnings have also been harvested to your wallet'))
          setPendingTx(false)
  ...
  const handleDeposit = async (convertedStakeAmount: BigNumber) => {
    ...
    try {
      // .toString() being called to fix a BigNumber error in prod
      // as suggested here https://github.com/ChainSafe/web3.js/issues/2077
      const tx = await dexTokenVaultContract.deposit(convertedStakeAmount.toString(), callOptions)
      const receipt = await tx.wait()
      if (receipt.status) {
